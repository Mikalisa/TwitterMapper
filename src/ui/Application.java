    ...
    private void UiInitialization() {
        ...
        map().addMouseMotionListener(new MouseAdapter() {
            ...
            public void mouseMoved(MouseEvent e) {
                ...
                mapMarkerList = getMarkersCovering(pos, pixelWidth(p));


                // TODO: Use the following method to set the text that appears at the mouse cursor
                for(MapMarker mp: mapMarkerList){
                    MapMarkerImage marker = (MapMarkerImage) mp;
                    Status s = marker.getStatus();
                    if (!mapMarkerList.isEmpty()) {
                        map().setToolTipText("<html>" + "<style>" + "body { background-color: #FFC300 ; height: 60px; width: 300px;}" + "</style>"+
                                "<body>" + "<img src="
                                + s.getUser().getProfileImageURL() + ">"  + s.getText() + "</body>" +"</html>");
                    }
